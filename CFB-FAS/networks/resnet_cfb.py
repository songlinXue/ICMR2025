...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, use_cnsn=True):
        super(BasicBlock, self).__init__()
        self.conv1 = conv3x3(inplanes, planes, stride)
        self.bn1 = normalization(planes)
        self.relu = nn.ReLU(inplace=False)
        ...
        if use_cnsn:
            crossnorm = CrossNorm()  # 默认参数
            selfnorm = SelfNorm(chan_num=planes)
            self.cnsn = CNSN(crossnorm, selfnorm)
        else:
            self.cnsn = None
