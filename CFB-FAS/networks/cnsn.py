...
class CNSN(nn.Module):
    ...
    def _initialize_weights(self):
        if hasattr(self.crossnorm, 'g_fc'):
            nn.init.kaiming_normal_(self.crossnorm.g_fc.weight, mode='fan_out', nonlinearity='relu')
            if self.crossnorm.g_fc.bias is not None:
                nn.init.constant_(self.crossnorm.g_fc.bias, 0)

        if hasattr(self.selfnorm, 'g_fc'):
            nn.init.kaiming_normal_(self.selfnorm.g_fc.weight, mode='fan_out', nonlinearity='relu')
            if self.selfnorm.g_fc.bias is not None:
                nn.init.constant_(self.selfnorm.g_fc.bias, 0)
        '''
