...
from loss import aggregate_real_features
...
def main(args):
    ...
    triplet_loss = TripletLoss(margin=args.triplet_margin).cuda()

    center_loss = CenterLoss(2, 512, use_gpu=True).cuda()

    def sample_triplets(feat, label):
        ...
        for i in range(batch_size):
            ...
            positive_indices = torch.where(label == anchor_label)[0]
            negative_indices = torch.where(label != anchor_label)[0]
            
            if positive_indices.size(0) > 1 and negative_indices.size(0) > 0:
                positive_index = positive_indices[torch.randint(0, positive_indices.size(0), (1,))][0]
                negative_index = negative_indices[torch.randint(0, negative_indices.size(0), (1,))][0]
                
                triplets.append((i, positive_index, negative_index))
        
        if len(triplets) == 0:
            return None, None, None
        
        triplets = torch.tensor(triplets, dtype=torch.long).cuda()
        triplet_anchor = feat[triplets[:, 0]]
        triplet_positive = feat[triplets[:, 1]]
        triplet_negative = feat[triplets[:, 2]]
        
        return triplet_anchor, triplet_positive, triplet_negative
    ...
    for epoch in range(args.start_epoch, args.num_epochs):
        ...
        for i, sample_batched in enumerate(train_loader):
            ...
            try:
                real_center = aggregate_real_features(feat_normed, label2)

                real_features = feat_normed[label2 == 1]
                if real_features.size(0) > 0:  
                    loss_aggregation = ((real_features - real_center) ** 2).mean()
                else:
                    loss_aggregation = torch.tensor(0.0, device=features.device, dtype=features.dtype) 
            except Exception as e:
                print(f"An error occurred: {e}")

            triplet_anchor, triplet_positive, triplet_negative = sample_triplets(feat, label2)

            if triplet_anchor is not None and triplet_positive is not None and triplet_negative is not None:
                triplet_l = triplet_loss(triplet_anchor, triplet_positive, triplet_negative)
            else:
                triplet_l = torch.zeros(1).cuda()

            triplet_l = triplet_loss(triplet_anchor, triplet_positive, triplet_negative)

            center_l = center_loss(feat_normed, label2)

            loss_all = cls_loss + args.feat_loss_weight * feat_loss + 0.01 * loss_aggregation + args.triplet_weight * triplet_l + args.center_weight * center_l
            ...
            if args.alpha == 0:
                angle = -1.0  
            elif epoch >= args.align_epoch and args.align == 'v4':
                angle = model.update_weight_v4(alpha=args.alpha)
            else:
                angle = -1.0
...
def parse_args():
    ...
    parser.add_argument('--scale', type=str, default='0.999', help='')
    parser.add_argument('--feat_loss', type=str, default='supcon', help='')
    parser.add_argument('--feat_loss_weight', type=float, default=0.5, help='')
    ...
    parser.add_argument('--triplet_weight', type=float, default=0.01, help='Weight for triplet loss in the total loss function')
    parser.add_argument('--center_weight', type=float, default=0.005, help='Weight for center loss in the total loss function')
    parser.add_argument('--num_classes', type=int, default=2, help='number of classes in the dataset')
    parser.add_argument('--feat_dim', type=int, default=512, help='dimension of feature vector from the model')
